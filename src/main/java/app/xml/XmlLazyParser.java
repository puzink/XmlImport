 ...
 * закрывающие элементы которых были прочитаны, и символы отступа{@link Character#isWhitespace(char)}
 * не записываются в тело узла, пока не будет найден первый символ, не являющийся отступом.
 *
 * В случае возникновения ошибки во время работы парсера и
 * продолжения работы с ним будет воссоздаваться первая cлучившаяся ошибка.
 *
 * Парсер не работает с xml-файлами, в которых есть пролог.
 */
